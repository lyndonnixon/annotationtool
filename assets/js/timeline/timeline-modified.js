...
links.Timeline.prototype.redrawAxis = function() {
    ...
    while (!step.end()) {
        ...
        if (options.showMinorLabels) {
            this.redrawAxisMinorText(x, step.getLabelMinor(), step.getLabelMajor());
        }
 ...
 * @param {String} text
 */
links.Timeline.prototype.redrawAxisMinorText = function (x, text, major) {
	/** MODIFIED - mba
	Show always major labels
	**/
    ...
    label.childNodes[0].nodeValue = major;
 ...
 * @param {Object} item
 */
links.Timeline.prototype.updateEventRange = function(item, num) {
    if (item.dom) {
        var divBox = item.dom;
        ...
        divBox.firstChild.innerHTML = item.content;
				...
				/** MODIFIED - mba
						Adds element number as class to item
				**/
				// console.log(num + ': ' + item.content);

        // update class
        divBox.className = "timeline-event timeline-event-range item-" + num;
				if (this.data.getValue(num, 17) != 1) {
					divBox.className += " timeline-event-hidden";
				}
				else {
					divBox.className += " timeline-event-visible";
				}
				if (this.data.getValue(num, 16) == 'http://www.w3.org/ns/openannotation/extensions/Bookmark') {
					// type-bookmark
					divBox.className += " type-bookmark";
				}
				if (!isNaN(this.data.getValue(num, 9)) && (this.data.getValue(num, 9) != '') && (this.data.getValue(num, 9) != null)) {
					// spatial-region-set
					divBox.className += " spatial-region-set";
				}
				
				// item.dom.setAttribute('id', "item-" + num);
 ...
 *
 *	MODIFIED mba ****************************
 *	Edit button for editing annotation source via popup added
 *	
 */
links.Timeline.prototype.redrawDeleteButton = function () {
    ...
    if (this.selection) {
        ...
        deleteButton.style.display = '';
				...
				editButton.style.top = (top + 18) + 'px';
				editButton.style.display = '';
				editButton.textContent = index;
